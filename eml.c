...
void new_parse_header_t(char* emlString, header_t* tht) {
...
void new_parse_header(char* emlString) {
    ...
    while (current_postition < emlstringlen) {
        ...
        case (int)',':
            ...
            break;
        case (int)'\"':
            new_parse_header_t(emlString, &tht); // Pass control to new_parse_header_t()
            break;
        default:
            exit(-1);
            break;
...
void new_parse_single_t(char *emlString, single_t *tst) {
...
void new_parse(char* emlString) {
    ...
    while (current_postition < emlstringlen) {
        ...
        case (int)'\"': // Give control to new_parse_single_t()
            new_parse_single_t(emlString, &tst);
            break;
        case (int)';':
            printf("End token\n");
            ++current_postition;
...
// Maybe have a "inside super" var, bc ":" does not terminate inside a super
